...
입력창 위 **회의 소집** 모드 선택 → 안건 입력 → 전달. 예:

> 환자 수 감소 대응 (참석: 상담실장, 운영매니저)

참석자를 안 적으면 안건 키워드로 알아서 고릅니다. 직원들이 비서실에 모여 1라운드 발표 → 2라운드 반박 → 비서실장 결론 순으로 진행되고, 결과 카드의 **회의록 markdown 복사** 버튼으로 볼트(`02_AI_Wiki/meetings/`)에 붙여넣을 수 있습니다. Claude·Gemini 키 모두 동작하며 회의 1회에 API 호출 5번이 나갑니다.
...
기본은 브라우저 localStorage(그 기기에만 저장)입니다. 여러 기기에서 학습·기억을 이어가려면 헤더의 **☁ 동기화** 버튼을 쓰세요:

1. **클라우드 자동 동기화 (추천, 어디서든 자동)** — 원장님 소유의 무료 **Supabase** 프로젝트에 저장.
   - Supabase에서 무료 프로젝트 생성 → `Settings → API`에서 **Project URL**과 **anon public key** 복사.
   - `SQL Editor`에 아래 한 번 실행:
     ```sql
     create table office_state (id text primary key, data jsonb, updated_at timestamptz);
     ...
     ```
   - ☁ 동기화 → 1번 → URL·키·이름표 입력. 이후 **같은 값**을 넣은 어느 기기·브라우저에서든 기억이 자동으로 이어집니다. (제3자 트래커가 아니라 원장님 DB)
2. **파일 내보내기/가져오기 (설정 0, 가장 안전)** — ☁ 동기화 → 2번으로 파일 저장 → 다른 기기에서 3번으로 불러오기. 파일을 옵시디언 볼트에 두면 볼트 동기화로 함께 이동합니다.

동기화 이름표(tag)를 다르게 하면 사람별로 기억을 분리할 수 있고, 브랜드(뷰티파크/슬로보다)는 자동으로 분리됩니다.

## 데이터

업무 이력·기억·일지는 브라우저 localStorage에 저장되고, 위 동기화를 켜면 지정한 Supabase(원장님 소유)에도 저장됩니다. API 키는 localStorage에만 저장되므로 공용 PC에서는 사용 후 비우세요.
